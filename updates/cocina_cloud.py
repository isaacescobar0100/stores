...
class LoginWindow:
    ...
    def _login(self):
        ...
        if result and result.get('success'):
            ...
            self.status_label.configure(text=result.get('error', 'Error de conexion'))
...
class CocinaApp:
    ...
    def _iniciar_actualizacion(self):
        ...
        self.root.after(100, self._cargar_pedidos)
    ...
    def _mostrar_detalles_pedido(self, pedido):
        ...
        fecha = pedido.get('fecha_hora', '')
        if fecha:
            ctk.CTkLabel(cliente_info, text=f"Fecha: {fecha[:16] if len(fecha) > 16 else fecha}", font=ctk.CTkFont(size=14), text_color=Theme.TEXT).pack(anchor='w', pady=2)
...
def main():
    ...
    login_window = ctk.CTkToplevel(login_root)
    login_window.protocol("WM_DELETE_WINDOW", login_root.destroy)
    LoginWindow(login_window, on_login)
    login_root.mainloop()
